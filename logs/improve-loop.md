...
### [轮次 21 · T2] 引用核验「正文零引用」不再给虚假绿色安心（招牌合规功能的第三态）
- 来源：第二次收敛再确认派独立 agent，确认 BODY_IDS 统一干净彻底后，最后一遍扫描发现的**唯一**新缺口——与刚修的 S2「逃逸核验」同类、但针对不同路径：`verify_citations` 在正文一处文献引用标记都没提取到时返回 `{total:0, unverified:[]}`，前端 `RationaleModule` 仅以 `unverified.length===0` 判成功，于是一份**零引用**的立项依据反而显示绿色「✓ 正文 0 处引用均来自真实文献」——虚假安心，恰好打击本工具「防编造/合规第一」的招牌。
- 改进：`RationaleModule` 引用核验改三态：`total===0`→中性灰提示「未检测到文献引用标记，请确认是否已据真实文献充分引用」；`total>0 && unverified.length===0`→绿色（不变）；有 unverified→警示（不变）。新增 `.verify-note` 中性样式。后端契约已由 `test_rationale` 的「无引用时 total=0」守卫。
- 验证：build=✅ 真实测试=✅ Playwright **59/59**（mock 含引用走绿色分支「出现引用核验区」无回归；新中性分支为纯展示条件，零回归风险）。
- 来源：本轮第三次 agent 调研（最终收敛确认）。**不在** logs 清单。
- 提交：`da03c71`
...
收敛验证全绿：后端 mock `[OK]`、`test_prompts` 31/31、`test_formatting` 9/9、`test_literature` 24/24、`test_rationale` 13/13、Playwright e2e **59/59**。

**重启动因**：用户再次发起 `/improve-loop`。第一次派 agent 以「挑剔产品评审 + 新手用户」视角深读全栈，找出 6 条有代码证据的方向（3 条高价值正确性 bug + 3 条低优先），全部落地。两次独立「收敛再确认」各又抓出 1 条（均为前序改动引入/遗漏的一致性收尾），也已落地。

本轮共落地 **8 个方向**（每个均：调研→实现→真实浏览器 e2e + 后端离线单测→日志→commit+push）：
1. `2df52a5` 项目摘要篇幅仪表系统性误报「偏长」修复（只测中文摘要段）
2. `1c7b1cb` 选题诊断不再喂给摘要凝练/全文评审（修内部口径矛盾）
3. `1b7be68` 润色稿不重复计入 30 页正文红线（约 2x 高估修复）
4. `1f0ed65` 引用核验覆盖 Semantic Scholar 链接（堵逃逸核验）
5. `36d23dd` 非字符串输入不再裸 500 + 未配置密钥侧边栏「未就绪」
6. `bb2c1d6` 统一「正文」口径单一真源 BODY_IDS（闭合方向 2/3 自引入的半截不一致）
7. `da03c71` 引用核验「正文零引用」改中性提示（不再虚假绿色安心）

> 编号 1–5 为首轮 agent 的 6 条方向（其中两条文档/UX 合并为 5），6–7 为两轮再确认的收尾。

**收敛判断**：三次独立调研（首轮 6 条 → 再确认①仅 BODY_IDS 一致性 → 再确认②仅零引用第三态）逐次收敛、每次只剩更小且同主线（防编造/一致性）的收尾项，现已全部落地；第三次 agent 全栈扫描后明确「除此一项外无其他高价值正确性/合规/体验缺口」。**已知遗留 bug：0。**

### 本轮主线
两条暗线贯穿 8 个方向：① **2026 改版引入的「宣称多于交付」与口径漂移**——篇幅仪表、正文集合在多处被重复定义且各不相同，本轮收敛为单一真源 `BODY_IDS`（页数 == 喂 LLM == 送评审，严格一致）；② **招牌「防编造/合规」功能的核验盲区**——S2 链接逃逸（方向 4）与零引用虚假绿色（方向 7）两条不同路径都会给虚假安心，均已堵上。

### 刻意不做（本轮复核仍判低价值）
- `_mock_flow` 状态串仍写「PubMed」：仅演示模式文案，非真实链路。
- 模块 03 UI 称「研究方案」而 2026 官方块名「研究内容」：`build_scheme` 已在提示词内说明三板块/自主组织/以当年指南为准，UI 改名属主观调优。
- 健康检查首探成功后不再重探：桌面/局域网后端常驻，历史已正当否决。
- `polish` 在「汇总导出工作包」里仍作为独立一节导出：导出=工作包框架下自洽（与「喂 LLM 正文」「页数」口径已分离），纯定位差异、低 ROI。

累计（含历史）：后端 4 个离线单测文件（31/9/24/13）+ Playwright e2e 59 项；7 个模块覆盖 2026 正文三板块。如 NSFC 再出新政策/指南，可针对性再起一轮。
